...
//			return new SemiMonthlyView();
	...
	class WeeklyView extends AbstractView {		
		...
		public void createNodes(LinearLayout layout) {
			...
			daysOfWeekText = x.addListNode2(layout, R.id.recurrence_pattern,R.drawable.ic_today, R.string.recurrence_weekly_days, daysToString());
		}
	...
	abstract class AbstractMonthlyView extends AbstractView {		
		...
		public void createNodes(LinearLayout layout) {
			...
			for (int i=0; i<num; i++) {
				...
				patternText[i] = x.addListNode2(layout, 100+i, R.drawable.ic_today,R.string.recurrence_monthly_pattern, activity.getString(pattern[i].titleId));
				switch (pattern[i]) {
				case EVERY_NTH_DAY:
					...
					break;
				case SPECIFIC_DAY:
					specificDayText[i] = x.addListNode2(layout, 200+i,R.drawable.ic_today, R.string.recurrence_monthly_specific_day, specificDayStr(i));
					break;
				}				
			}
