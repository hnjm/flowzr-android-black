...
public class TransactionInfoDialog {
    ...
    public void show(BlotterFragment blotterActivity, long transactionId) {
        TransactionInfo ti = em.getTransactionInfo(transactionId);
        if (ti == null) {
            Toast t = Toast.makeText(blotterActivity.getActivity(), R.string.no_transaction_found, Toast.LENGTH_LONG);
            t.show();
            return;
        }
        if (ti.parentId > 0) {
    ...
    private void showDialog(final BlotterFragment blotterActivity, final long transactionId, final View v, View titleView) {
        final Dialog d = new AlertDialog.Builder(blotterActivity.getActivity())
