...
public class CategoryBuilder {
    ...
    public static Map<String, Category> createDefaultHierarchy(DatabaseAdapter db) {
        ...
        new CategoryBuilder(db).withParent(a1).withTitle("AA1")
                .withAttributes(
                        com.flowzr.test.test.AttributeBuilder.withDb(db).createTextAttribute("attr1"),
                        com.flowzr.test.test.AttributeBuilder.withDb(db).createNumberAttribute("attr2")
                ).create();
    ...
    private static Map<String, Category> allCategoriesAsMap(DatabaseAdapter db) {
        ...
        List<Category> categories = db.getCategoriesList(true);
